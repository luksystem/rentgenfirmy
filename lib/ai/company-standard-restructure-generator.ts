...
import type { CompanyStandardStep } from "@/lib/standards/types";

export const COMPANY_STANDARD_RESTRUCTURE_MAX_INPUT_CHARS = 6_000;

function buildPrompt(input: { draftText: string; instructions: string }) {
  const instructions = input.instructions.trim() || DEFAULT_COMPANY_STANDARD_RESTRUCTURE_INSTRUCTIONS;
...
Odpowiedz WYŁĄCZNIE poprawnym JSON w formacie:
{
  ...
  "tipsHtml": "wskazówki/uwagi/wyjątki (może być pusty string, jeśli brak)"
}`;
}

export type CompanyStandardRestructureResult = {
  contextHtml: string;
  steps: CompanyStandardStep[];
  tipsHtml: string;
};

export async function generateCompanyStandardRestructure(input: {
  draftText: string;
  instructions?: string;
      ...
      messages: [
        {
          role: "system",
          content:
            "Jesteś redaktorem technicznym porządkującym wewnętrzne standardy i procedury firmowe. Odpowiadasz wyłącznie w formacie JSON, po polsku.",
        },
        {
          role: "user",
          content: buildPrompt({
            draftText: plain.slice(0, COMPANY_STANDARD_RESTRUCTURE_MAX_INPUT_CHARS),
            instructions: input.instructions ?? DEFAULT_COMPANY_STANDARD_RESTRUCTURE_INSTRUCTIONS,
          }),
        },
      ],
    }),
