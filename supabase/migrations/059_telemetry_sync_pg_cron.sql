-- Harmonogram synchronizacji telemetrii Loxone (pg_cron + pg_net → istniejące API Next.js).
-- Logika odczytu pozostaje w /api/cron/telemetry-sync na Vercel.
--
-- Po uruchomieniu migracji 060 wstaw URL i sekret (ten sam CRON_SECRET co na Vercel):
--
...
select cron.schedule(
  'telemetry-sync',
  '*/5 * * * *',
  'select public.trigger_telemetry_sync_cron();'
);
