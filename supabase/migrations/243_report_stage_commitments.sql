-- Krok A A7 (docs/08 D27 2.4) - kalendarz zobowiazan etapu dla managera: co musi byc gotowe w
-- najblizszych N dni (domyslnie 21), kto odpowiada, co zrobione, gdzie rozbieznosc planu z
-- terminem wynikajacym. Wzorzec jak report_rot_items/report_stage_health - zero odwolan do
-- konkretnego elementu/etapu, wszystko po atrybutach (process_items.lead_days).
...
as $$
  ...
  select
    ci.project_id, ci.project_name, ci.stage_id, ci.stage_title, ci.item_id, ci.title,
    ci.termin_wynikajacy, ci.data_planowana, ci.data_ukonczenia,
...
comment on function public.report_stage_commitments is
  'Krok A A7 (docs/08 D27 2.4) - kalendarz zobowiazan aktywnego etapu kazdego projektu: elementy z '
  'lead_days w oknie p_horizon_days (domyslnie 21), kto odpowiada (assignee_id lub macierz '
  'process_stage_role_responsibility -> project_role_slot), czy zrobione, gdzie data_planowana '
  'przekracza termin_wynikajacy (status=rozbieznosc). Zero odwolan do konkretnego etapu/elementu.';
