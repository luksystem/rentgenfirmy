-- Nowy kind powiadomienia: instalator oznaczył moduł dokumentacji (Rolety/Przyciski/Alarm/HVAC/
-- RACK) jako zakończony, osoba odpowiedzialna za etap dostaje o tym informację (push + w apce) —
-- ten sam wzorzec co switchboard_wiring_completed.
...
alter table user_notifications add constraint user_notifications_kind_check check (
  kind = any (array[
    ...
    'leave_commitment_impact','employee_report_classified','employee_report_accepted',
    'employee_report_completed','employee_report_closed','employee_report_urgent',
    'requisition_order_overdue','process_snapshot_uploaded',
    'warranty_review_due','leave_substitution_gap','leave_substitution_client_draft',
    'leave_substitution_proposed','switchboard_wiring_completed','documentation_module_completed'
  ])
);
