...
| D26 | Podział przez kliknięcie na kafelku Gantta | Ikonka nożyczek na bloku uzbraja „tryb podziału” (`splitArmed`, stan lokalny `GanttBlock`) — w tym trybie `beginDrag` jest wyłączone (early return), a każde kliknięcie na kafelku liczy dzień pod kursorem (ta sama matematyka pikseli co przy `onDoubleClick` tworzenia nowego elementu, przyciągnięta do siatki dni wg aktualnego zoomu — `snapDays`) i rysuje przerywaną kreskę + mały popover z potwierdzeniem (✓/✕) tuż nad kreską. Brak nowej biblioteki popoverów — pozycjonowanie absolutne względem bloku, `stopPropagation` chroni popover od zamknięcia przez klik w tło. Kandydacki punkt jest przycinany do otwartego przedziału (`startAt`, `endAt`); jeśli przydział jest krótszy niż jeden pełny dzień (przy zoomie miesięcznym) błąd pojawia się w popoverze, a nie przez zablokowanie kliknięcia. |
| D27 | Scalanie (odwracanie podziału) | `mergeLinkedGroupItems()` (repozytorium) — nie „undo” historii zmian, a jawna operacja: zachowuje najstarszą część (najwcześniejszy `startAt`) jako nośnik, rozciąga jej zakres na min/max całej grupy, sumuje `plannedHours` (tylko gdy WSZYSTKIE części mają podane godziny — inaczej `null`, bez zgadywania), łączy uczestników (deduplikacja po `userId`, pierwsze wystąpienie wygrywa) i usuwa pozostałe części. Wywoływane z panelu edycji (przycisk „Scal części” z inline potwierdzeniem tak/nie) — nie z Gantta, bo scalanie dotyczy całej grupy, a nie jednego kliknięcia na kafelku. |
| D28 | „Zależność pociętych” — kaskadowe przesunięcie | Jeden boolean per-element (`resource_plan_items.shift_with_linked_group`), ustawiany naraz na WSZYSTKICH częściach grupy (`setLinkedGroupShiftEnabled`) — to ustawienie grupy, nie pojedynczej części, więc musi być zsynchronizowane, inaczej zachowanie zależałoby od tego, którą część aktualnie edytujemy. W Gantcie (`commitDrag`) po zapisaniu zmiany danej części, gdy flaga jest włączona, liczona jest delta `nextEndAt - item.endAt` i o tę samą wartość przesuwane są WSZYSTKIE części z `startAt` późniejszym niż oryginalny `startAt` przesuwanej części — jedna formuła działa jednolicie dla przeciągania całości („move”, oba końce przesuwają się równo) i rozciągania prawej krawędzi („resize-end”, zmienia się tylko koniec); rozciąganie lewej krawędzi („resize-start”) naturalnie nie kaskaduje, bo delta końca wynosi 0. Odstępy czasu między częściami są zachowane, bo przesunięcie jest identyczne dla wszystkich kolejnych części, nie przeliczane od nowa. |

## 8. Co dalej

Wszystkie 8 etapów pierwotnego planu są wdrożone, wraz z późniejszym rozszerzeniem o % zaangażowania
uczestników, podział przydziałów w czasie, podział przez kliknięcie na kafelku, scalanie i kaskadowe
przesunięcie pociętych części (D23–D28). Pozostały dług techniczny i możliwe dalsze kierunki (testy
automatyczne walidacji/sugestii, API route'y serwerowe, ewentualny dostawca AI, wizualne łączenie
części podzielonego przydziału linią w Gantcie) opisane w `STAN_WDROZENIA.md` §„Braki i dług
techniczny”.
